...
source: "fireflo-control-panel lib/period-close.ts @ 0.7.3"
---
...
What the row *is* load-bearing for is the window. `PostpaidAllowance` reads
`max(created_at) WHERE entry_type = 'BILL'` as the start of the unbilled period — so **closing is what
